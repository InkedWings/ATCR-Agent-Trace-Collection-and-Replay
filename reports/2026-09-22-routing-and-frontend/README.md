# 新结果合并分析：前端资源对照与 OpenClaw RR 扩展
...
|---|---:|---:|---:|---:|
| 1+8，build=4（旧对照） | 5,259.45 | 18.66 | 96.64% | 95.79% |
| 2+8，build=2/前端（新） | 5,690.48 | 20.39 | 95.57% | 44.98% |
| 1+8，build=8（新） | 5,204.49 | 18.49 | 96.29% | 98.09% |

![Frontend controls](figures/01_frontend_controls.png)

**2+8 相对 1+8/build4：输出 +8.20%，任务 +9.29%。** 两个前端 CPU 分别为 44.83%、45.13%，均未接近饱和。实际复用略低于旧对照，吞吐却更高，所以本次提升不需要用“cache hit 变好”解释。

**1+8/build8：输出变化 -1.05%，未看到改善。** CPU 升至 98.09%，平均 runnable processes 从 124 增至 168；放宽创建名额后，前端仍没有足够资源让创建和工具执行同时更快。

| 配置 | 等待 build 名额 mean / p95，s | 实际 build mean / p95，s | 前/后 45 min 的等待均值，s |
|---|---:|---:|---:|
| 1+8，build=4（旧对照） | 97.29 / 350.27 | 12.72 / 18.96 | 65.61 → 129.15 |
| 2+8，build=2/前端（新） | 0.94 / 5.01 | 5.32 / 7.15 | 1.08 → 0.80 |
| 1+8，build=8（新） | 11.09 / 57.17 | 21.04 / 31.45 | 2.73 → 19.56 |
...
|---|---:|---:|---:|---:|---:|
| 1+8，build=4（旧对照） | 110.65 | 485.18 | 194.72 | 8.00 | 798.55 |
| 2+8，build=2/前端（新） | 6.49 | 624.42 | 102.62 | 3.51 | 737.05 |
| 1+8，build=8（新） | 33.07 | 498.83 | 257.44 | 13.96 | 803.31 |

![Lifecycle and build distributions](figures/02_frontend_time_breakdown.png)

2+8 的工具阶段约缩短 47.3%；build8 则增长约 32.2%。2+8 每后端平均 running 从 18.89 增至 26.45；build8 为 19.17。2+8 的 LLM 墙钟阶段增长，符合更多并发从 setup/tools 转入后端的现象，不能把它单独解释为模型变慢。任务整体生命周期从 798.55 s 降至 737.05 s。
...
|---|---:|---:|---:|
| 1+8，build=4（旧对照） | 152,233 | 1,194 | 0.78% |
| 2+8，build=2/前端（新） | 165,305 | 718 | 0.43% |
| 1+8，build=8（新） | 151,111 | 1,413 | 0.94% |
...
|---|---|---:|---:|---:|
| OpenClaw 1+4 RR | 完整、非稳态 | 614.59 | 9.10 | 63.48% |
| OpenClaw 1+8 RR | 窗口完整、warmup 失败且非稳态 | 1,247.03 | 18.70 | 61.77% |
| OpenClaw 1+4 cache-aware | 完整 | 1,618.49 | 23.60 | 90.58% |
| OpenClaw 1+8 cache-aware | 缺测 | — | — | — |
| mini-SWE 1+4 RR | 窗口恢复、drain 超时 | 775.60 | 2.61 | 59.49% |
| mini-SWE 1+8 RR | 缺测 | — | — | — |
| mini-SWE 1+4 cache-aware | 完整 | 2,412.67 | 8.73 | 88.58% |
| mini-SWE 1+8 cache-aware | 缺测 | — | — | — |

![Routing comparison](figures/04_routing_comparison.png)

OpenClaw RR 4→8：输出 **2.03×**，任务 **2.05×**。但 1+8 RR 的 1247.03 tokens/s 只有 1+8 sticky 实测的 **36.11%**，也低于 1+4 cache-aware 的 1618.49 tokens/s。Sticky 走直连、routing 走官方 router，二者作为部署性能参考，不能将差异全归于路由算法。
...
|---|---:|---:|---:|---:|---:|
| r00 | 2,006 | 68.17% | 2.41 | 0.06 | 4.60 |
| r01 | 2,006 | 67.29% | 2.75 | 0.06 | 5.06 |
| r02 | 2,006 | 68.14% | 2.42 | 0.05 | 4.50 |
| r03 | 2,006 | 68.85% | 2.72 | 0.10 | 5.18 |
| r04 | 2,006 | 69.10% | 2.30 | 0.08 | 4.46 |
| r05 | 2,006 | 69.26% | 2.35 | 0.06 | 4.49 |
| r06 | 2,006 | 68.90% | 2.37 | 0.07 | 4.56 |
| r07 | 2,006 | 13.90% | 39.82 | 61.85 | 188.26 |
